...
function updateTimerDisplay() {
    ...
    timerDisplay.textContent = `${hours}:${minutes}:${seconds}.00`.replace(/^0+:/, '');
}

function updatetotalTime(){
    if(!isRunning){
        totalTime = (hoursInput.value * 3600000) + (minutesInput.value * 60000) + (secondsInput.value * 1000);
    }
}
...
function update(){
    if(totalTime >= 0){
        ...
        timerDisplay.textContent = `${hours}:${minutes}:${seconds}.${milliseconds}`.replace(/^0+:/, '');
        totalTime-=10;
    }
